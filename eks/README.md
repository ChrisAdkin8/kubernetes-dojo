...
6. The OIDC provider allows AWS IAM to trust tokens issued by the Kubernetes API server. When a Pod's ServiceAccount has an `eks.amazonaws.com/role-arn` annotation, the [EKS Pod Identity webhook](https://docs.aws.amazon.com/eks/latest/userguide/pod-configuration.html) (a mutating admission webhook) adds the `AWS_ROLE_ARN` and `AWS_WEB_IDENTITY_TOKEN_FILE` environment variables and a projected service account token volume to the Pod. The AWS SDK exchanges this token with STS (`AssumeRoleWithWebIdentity`), which validates the token's signature against the cluster's OIDC endpoint. If valid, STS returns temporary credentials scoped to the annotated IAM role.
