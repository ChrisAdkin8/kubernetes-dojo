...
> **DNS propagation delay:** Even after the hostname appears in `EXTERNAL-IP`, the AWS ELB DNS record can take **1–3 minutes** to propagate. If `curl` returns `Could not resolve host`, confirm DNS is ready first:
>
> ```bash
> nslookup <EXTERNAL-IP-HOSTNAME>
> ```
>
> Once `nslookup` returns an IP address, `curl` will succeed.
